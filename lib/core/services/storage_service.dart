...
class StorageService extends ChangeNotifier {
  ...
  String get usageFormatted => '${_formatBytes(_storageUsedBytes)} / ${_currentTier.storageLimitFormatted}';
  ...
  static List<SubscriptionInfo> get subscriptionPlans => [
    ...
    SubscriptionInfo(
      ...
      storage: '50 GB',
      priceEuros: 4.99,
      features: ['Everything in Basic', '50 GB storage', 'Advanced features'],
    ),
    SubscriptionInfo(
      ...
      storage: '200 GB',
      priceEuros: 9.99,
      features: ['Everything in Standard', '200 GB storage', 'Priority support'],
    ),
  ];
  ...
  Future<void> loadStorageInfo(String userId) async {
    ...
    try {
      ...
      if (doc.exists) {
        final data = doc.data()!;
        _currentTier = SubscriptionTier.values[data['subscriptionTier'] as int? ?? 0];
  ...
  Future<String?> uploadFile({
    ...
    if (_storageUsedBytes + fileSize > storageLimitBytes) {
      _errorMessage = 'Storage quota exceeded. Upgrade your plan for more space.';
      notifyListeners();
      return null;
    }
...
class SubscriptionInfo {
  ...
  String get priceFormatted => priceEuros == 0 
      ? 'Free' 
      : '€${priceEuros.toStringAsFixed(2)}/month';
}
