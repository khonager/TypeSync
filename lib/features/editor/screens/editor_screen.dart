...
import '../widgets/editor_command_palette.dart';
...
class _EditorScreenState extends State<EditorScreen>
  ...
  late final AttachmentPreferencesService _attachmentPreferencesService;
  late final EditorCommandService _editorCommandService;
  bool _isEditorCommandServiceListening = false;

  // Auto-save timer
  Timer? _saveTimer;
  Timer? _contentMaintenanceTimer;
  ...
  final ValueNotifier<EditorCommandMenuState?> _editorCommandMenu =
      ValueNotifier(null);
  ...
  void initState() {
    ...
    _attachmentPreferencesService =
        context.read<AttachmentPreferencesService>();
    _editorCommandService = context.read<EditorCommandService>();
    _attachmentPreferencesService
        .addListener(_handleAttachmentPreferencesChanged);
  ...
  Future<void> _initializeEditor() async {
    ...
    _editorCommandService.addListener(_refreshEditorCommandMenu);
    _isEditorCommandServiceListening = true;
    _refreshEditorCommandMenu();
  ...
  void _onContentChanged() {
    _refreshEditorCommandMenu();
    if (_isUpdatingFromExternal || _isApplyingChecklistMetadata) return;

    // Quill notifies listeners synchronously, before the newly typed character
    // can be painted. Keep this path deliberately small: whole-document work
    // here makes characters appear late and causes queued input to arrive in
    // visible bursts on longer notes.
    _scheduleContentMaintenance();
    _scheduleSave();
    _scheduleCaretOffsetPersist();
  }

  void _scheduleContentMaintenance() {
    _contentMaintenanceTimer?.cancel();
    _contentMaintenanceTimer = Timer(
      const Duration(milliseconds: 250),
      _runContentMaintenance,
    );
  }

  void _runContentMaintenance() {
    _contentMaintenanceTimer?.cancel();
    _contentMaintenanceTimer = null;
  ...
  bool _tryExpandEditorCommand(int insertionOffset) {
    ...
    final expansion = commandExpansionAt(
      ...
      resolveTemplate: (trigger) =>
          _editorCommandService.commandForTrigger(trigger)?.template,
    );
  ...
  void _refreshEditorCommandMenu() {
    if (!mounted) return;
    final selection = _quillController.selection;
    if (!selection.isValid || !selection.isCollapsed) {
      _editorCommandMenu.value = null;
      return;
    }

    _editorCommandMenu.value = commandMenuStateAt(
      documentText: _quillController.document.toPlainText(),
      caretOffset: selection.baseOffset,
      commands: _editorCommandService.commands,
    );
  }

  void _insertCommandFromPalette(EditorCommand command) {
    final menuState = _editorCommandMenu.value;
    if (menuState == null ||
        !menuState.matches.any(
          (candidate) => candidate.trigger == command.trigger,
        )) {
      return;
    }
    ...
    try {
      ...
      _quillController.updateSelection(
        TextSelection.collapsed(
          offset: menuState.start + command.template.length,
        ),
        ChangeSource.local,
      );
    } finally {
  ...
  Future<void> _loadAttachmentPreferences() async {
    ...
    await _attachmentPreferencesService.load();
    await _attachmentPreferencesService.migrateLegacyPreferences(noteId);
    _applyAttachmentPreferences();
  }
  ...
  Future<void> _persistAttachmentPreferences() async {
    ...
    await _attachmentPreferencesService.update(
      noteId,
      AttachmentPreferences(
        expanded: _attachmentsExpanded,
        previewHidden: _hideAttachmentPreview,
      ),
    );
  }

  void _handleAttachmentPreferencesChanged() {
    if (!mounted) return;
    _applyAttachmentPreferences();
  }

  void _applyAttachmentPreferences() {
    final preferences = _attachmentPreferencesService.preferencesFor(_note?.id);
    if (_attachmentsExpanded == preferences.expanded &&
        _hideAttachmentPreview == preferences.previewHidden) {
      return;
    }
    setState(() {
      _attachmentsExpanded = preferences.expanded;
      _hideAttachmentPreview = preferences.previewHidden;
    });
  }
  ...
  void _scheduleSave() {
    _saveTimer?.cancel();
    // Leave enough idle time for the coalesced document maintenance pass to
    // finish first, so saved stats and checklist metadata remain current.
    _saveTimer = Timer(const Duration(milliseconds: 600), _saveNote);
  }

  Future<void> _saveNoteFromShortcut() async {
    _saveTimer?.cancel();
    if (_contentMaintenanceTimer != null) {
      _runContentMaintenance();
    }
    await _saveNote();
  }
  ...
  void _scheduleTitleSave(String title) {
    ...
    _titleSaveTimer = Timer(
      ...
      _flushPendingTitleSave,
    );
  }

  Future<void> _flushPendingTitleSave() async {
    ...
    _pendingTitle = null;
    await _updateTitle(title);
  }

  void _onTitleFocusChanged() {
    if (!_titleFocusNode.hasFocus) {
      unawaited(_flushPendingTitleSave());
    }
  }
  ...
  void dispose() {
    ...
    if (_isEditorCommandServiceListening) {
      _editorCommandService.removeListener(_refreshEditorCommandMenu);
    }
      ...
      child: DefaultTextStyle.merge(
        ...
        child: Stack(
          ...
          children: [
            ...
            ValueListenableBuilder<EditorCommandMenuState?>(
              valueListenable: _editorCommandMenu,
              builder: (context, menuState, _) {
                if (menuState == null) return const SizedBox.shrink();
                return Positioned.fill(
                  child: EditorCommandPalette(
                    state: menuState,
                    onSelected: _insertCommandFromPalette,
                  ),
                );
              },
            ),
